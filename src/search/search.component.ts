...
export class SearchComponent implements OnInit {
  ...
  options : string[] = [];
  person: person = {homeworld: 'Homeworld', name : 'Character name', gender : 'n/a',};
  filteredOptions: Observable<string[]>;

  constructor(private http: HttpClient) {}

  ngOnInit() {
    this.http.get<names[]>(`${environment.API_URL}`)
    .subscribe(names =>{
  ...
  submit(){
    this.http.get<[person]>(`${environment.API_URL}/search-people`, {
